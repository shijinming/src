...
/**
 * \ingroup UAVRouting
 * \brief The Routing table used by UAV routing protocol
 */
class RoutingTable
{
public:
...
public:
    ...
    void UpdateInfo(Ipv4Address ipv4, NodeInfo info);
    void UpdateMatrix ();
    void Dijkstra(uint32_t begin);
    void UpdateNeighbour();
    void GetListOfNodeInfo(std::map<Ipv4Address, NodeInfo> & nodeInfo);
    ...
    GetAddress() const
    {
      return m_localAdress;
    }

    bool
    LookupRoute(Ipv4Address addr, Ptr<Ipv4Route> & route);

    void
    AddNetDevice(Ipv4Address addr, Ptr<NetDevice> dev);

private:
    uint32_t m_nodeNum;
    Ipv4Address m_localAdress;
    double *m_distMatrix;
    std::map<Ipv4Address, NodeInfo> m_nodeInfo;
    std::map<Ipv4Address, Ptr<NetDevice> > m_netDevice;
    double m_validTime;
    uint32_t m_queueLen;
    double m_energy;
    double m_maxDis;
